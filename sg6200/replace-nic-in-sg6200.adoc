...
summary: 'Möglicherweise ist ein Austausch der Netzwerkkarte (NIC) im SGF6212 erforderlich, wenn sie nicht optimal funktioniert oder ausgefallen ist.' 
---
= Ersetzen Sie die interne Netzwerkkarte im SGF6212 oder SG6200-CN
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Möglicherweise müssen Sie die interne Netzwerkkarte (NIC) im SGF6212 oder SG6200-CN austauschen, wenn sie nicht optimal funktioniert oder ausgefallen ist.
...
CAUTION: Wenn Sie jemals eine ILM-Regel verwendet haben, die nur eine Kopie eines Objekts erstellt, müssen Sie die NIC während eines geplanten Wartungsfensters ersetzen, da während dieses Vorgangs vorübergehend der Zugriff auf diese Objekte verloren geht. Siehe Informationen über https://docs.netapp.com/us-en/storagegrid/ilm/why-you-should-not-use-single-copy-replication.html["Warum sollten Sie die Single-Copy-Replizierung nicht verwenden"^].

.Bevor Sie beginnen
* Sie haben die richtige Ersatz-NIC.
* Sie haben link:locating-sg6200-in-data-center.html["physisch platziertes SGF6212 Appliance oder SG6200-CN Controller"], an dem Sie die Netzwerkkarte im Rechenzentrum austauschen.
+
...
+
image:../media/drw_s2025_io_numbers_ieops-2554.svg["Position der Netzwerkkarten im Gerät"]
...
1
 a| 
NIC 1 in der Einsteckplatz-Riser-Baugruppe
 a| 
10/25-GbE Ethernet-Netzwerkanschlüsse für Grid- und Client-Netzwerke
...
2
 a| 
NIC 2 in der Einsteck-Riser-Baugruppe
 a| 
1/10-GbE Ethernet-Netzwerkanschlüsse für das Admin Network
...
3
 a| 
NIC 3 in der Einsteck-Riser-Baugruppe
 a| 
10/25-GbE Ethernet-Netzwerkanschlüsse für Grid- und Client-Netzwerke
...
NOTE: Einige Geräte unterstützen schnellere Netzwerk-Ports und verfügen möglicherweise über schnellere NICs.
