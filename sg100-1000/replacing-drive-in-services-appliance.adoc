...
---
= Ersetzen Sie ein Laufwerk in einem SG100 oder SG1000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Sie können den Grid Manager auch verwenden, um den Status der SSDs zu überwachen. Wählen Sie *Knoten*. Wählen Sie dann  `*Appliance Node*` > *Hardware*. Wenn ein Laufwerk ausgefallen ist, enthält das Feld Storage RAID Mode eine Meldung darüber, welches Laufwerk ausgefallen ist.
...
+
Das Laufwerk wird automatisch mit gespiegelten Daten des funktionierenden Laufwerks wiederhergestellt. Den Status der Wiederherstellung können Sie im Grid Manager überprüfen. Wählen Sie *Knoten*. Wählen Sie dann  `*Appliance Node*` > *Hardware*. Im Feld „Storage RAID Mode“ wird die Meldung „`rebuilding`“ angezeigt, bis das Laufwerk vollständig wiederhergestellt ist.
