...
summary: 'Wenn Sie während der Installation die Node-Verschlüsselung für die Appliance aktiviert haben, können Sie den Verschlüsselungsstatus aller Appliance-Nodes überwachen, einschließlich Details zur Node-Verschlüsselung und zum Key Management Server (KMS).' 
---
= Überwachen Sie die Knotenverschlüsselung für eine StorageGRID Appliance im Wartungsmodus
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Nach der Außerbetriebnahme eines Geräts, bei dem die Knotenverschlüsselung aktiviert war.
...
+
Der KMS-Verschlüsselungsschlüssel und alle Daten werden vom Knoten gelöscht und das Gerät wird neu gestartet.  Während des Neustarts der Appliance wird ein Dialogfeld angezeigt. Dieser Vorgang kann bis zu 20 Minuten dauern.
