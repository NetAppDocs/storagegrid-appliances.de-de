...
summary: 'Der Klonprozess kann mehrere Tage dauern, bis die Daten zwischen dem Quell-Node (Appliance, die ersetzt wird) und der Ziel-Appliance übertragen werden.' 
---
= Klonen eines StorageGRID Appliance-Knotens
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Wenn Probleme bestehen, die das Klonen verhindern, ist *Klonen starten* nicht aktiviert und Probleme, die Sie beheben müssen, werden als *Verbindungsstatus* aufgeführt. Diese Probleme werden auf der StorageGRID Appliance Installer-Startseite sowohl des Quellknotens als auch des Zielgeräts angezeigt. Es wird jeweils nur ein Problem angezeigt und der Status wird automatisch aktualisiert, wenn sich die Bedingungen ändern. Beheben Sie alle Klonprobleme, um *Klonen starten* zu aktivieren.
...
NOTE: Wenn am Ende des Klonprozesses ein IP-Adresskonflikt auftritt, sollte sichergestellt werden, dass für keine der MAC-Adressen von Ports, die auf dem Quellknoten statische IP-Adressen hatten und auf dem Zielknoten für DHCP aktiviert wurden, permanente DHCP-Leases vergeben wurden. Das Entfernen oder Aktualisieren der permanenten DHCP-Leases auf dem DHCP-Server behebt diesen Konflikt.
