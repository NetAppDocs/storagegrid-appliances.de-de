...
---
= Verkabeln Sie ein StorageGRID Appliance oder einen Controller
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** Ein bis vier Twinax-Kabel zum Anschließen der vier Netzwerk-Ports.
** Für den SG120 benötigen Sie ein bis vier SFP+ oder SFP28 Transceiver, wenn Sie optische Kabel für die Ports verwenden möchten.
** Für den SG1200, ein bis vier QSFP+, QSFP28 oder QSFP56 (nur 200GbE NIC) Transceiver, wenn Sie planen, optische Kabel für die Ports zu verwenden.
...
SG120-Portanschlüsse:

image::../media/sg120_connections.png[SG120-Rückseitenanschlüsse]

SG1200-Portanschlüsse:

image::../media/sg1200_connections.png[SG1200-Portanschlüsse]
...
* 10/25/40/100-GbE mit 100G NIC
* 10/25/50/100/200-GbE (nur 200GbE NIC)

 a| 
Stellen Sie eine Verbindung zum Grid-Netzwerk und zum Client-Netzwerk für StorageGRID her. Siehe link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes["Port-Bond-Modi (SG120 und SG1200)"]
...
[cols="2a,2a"]
|===
| SG120-Verbindungsgeschwindigkeit (GbE) | Erforderliche Ausrüstung 
...
[cols="2a,2a"]
|===
| SG1200-Verbindungsgeschwindigkeit (GbE) | Erforderliche Ausrüstung 
...
NOTE: Der physische Verbindungsstatus für Port 1 ist in der Software nicht verfügbar und muss zu diesem Zeitpunkt mithilfe der Status-LED am SG5800-Controller überprüft werden.
...
.Bevor Sie beginnen
* Nur für SG6200: Sie verfügen über das mit dem Gerät gelieferte 100GbE-zu-4x25GbE-Breakout-Kabel zum Anschluss der beiden Speichercontroller an den SG6200-CN-Controller.
* Sie verfügen über RJ-45-Ethernet-Kabel:
+
** Ein RJ-45-Kabel zum Anschließen des Management-Ports.
** Nur SG6200: Bis zu vier zusätzliche RJ-45-Ethernet-Kabel für die optionalen Ports, die Sie verwenden möchten, einschließlich des zweiten Admin-Ports und des BMC-Ports am SG6200-CN sowie der Wartungsports an jedem der beiden E4000-Controller.


* Sie haben eine der folgenden Optionen für die Netzwerkanschlüsse. Diese Gegenstände sind nicht im Lieferumfang des Geräts enthalten.
+
** Ein bis vier Twinax-Kabel zum Anschließen der vier Netzwerk-Ports.
** Ein bis acht SFP+, SFP28, QSFP+, QSFP28 oder QSFP56 Transceiver, wenn Sie optische Kabel für die Ports verwenden möchten.
...
.Über diese Aufgabe
Die folgenden Abbildungen zeigen die Ports auf der Rückseite des SGF6212 und die drei Controller im SG6200 Appliance. Das SG6200 Appliance enthält einen SG6200-CN Compute Controller oben und zwei E4000 Storage Controller unten.

SGF6212-Anschlüsse:

image::../media/sgf6212_connections.png[SGF6212 rückseitige Anschlüsse]

SG6260-Anschlüsse:

image::../media/sg6200_e4000_connections.png[SG6200-CN-zu-E4000-Verbindungen]
...
2
 a| 
Vier Netzwerkanschlüsse am SG6200-CN-Controller
 a| 
* Vier 10/25/40/100-GbE-Netzwerkanschlüsse oder 200-GbE mit optionaler 200G-NIC-SKU

 a| 
Stellen Sie eine Verbindung zum Grid-Netzwerk und zum Client-Netzwerk für StorageGRID. Siehe link:gathering-installation-information-sg6200.html#port-bond-modes["Port-Bond-Modi (SG6200)"]
...
Fünf Anschlüsse insgesamt
 a| 
* Ein 100GbE-Port am SG6200-CN
* Zwei 10/25-GbE-Ports auf jedem der Storage Controller

 a| 
Verbinden Sie jeden Speichercontroller mit dem SG6200-CN Controller.
...
. Verbinden Sie die beiden Interconnect-Ports jedes Storage-Controllers mit dem 100GbE-Port des SG6200-CN-Controllers mithilfe eines 100-GbE QSFP28-zu-4x25-GbE SFP28-Kabels.
. Verbinden Sie die Netzwerk-Ports des Geräts mit den entsprechenden Netzwerk-Switches über Twinax-Kabel oder optische Kabel und Transceiver.
+
[cols="2a,2a"]
|===
...
. Wenn Sie das Admin-Netzwerk für StorageGRID verwenden möchten, schließen Sie den Admin-Netzwerkport des Geräts über ein Ethernet-Kabel an das Admin-Netzwerk an.
. Falls Ihr Gerät über einen Erweiterungs-Shelf verfügt, siehe die link:../sg6200/adding-expansion-shelf-to-deployed-sg6260.html["Anleitung zum Hinzufügen eines Erweiterungsfachs zu einem bereits installierten SG6200"] für Verkabelungsinformationen.
