...
---
= Aktivieren Sie die Knoten- oder Laufwerksverschlüsselung für StorageGRID Appliances
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Wenn Sie den FIPS-Modus nach der Aktivierung der Knotenverschlüsselung aktivieren, wird außerdem das Modul NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 zur Verschlüsselung ruhender Daten verwendet. Siehe https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy["Wählen Sie eine Sicherheitsrichtlinie"^] für weitere Informationen.
...
CAUTION: Wenn eine Appliance mit aktivierter Knotenverschlüsselung installiert wird, wird ein temporärer Schlüssel zugewiesen.  Die Daten auf der Appliance sind erst geschützt, wenn die Appliance mit dem Key Management System (KMS) verbunden ist und ein KMS-Sicherheitsschlüssel festgelegt ist.  Weitere Informationen finden Sie im https://docs.netapp.com/us-en/storagegrid/admin/kms-overview-of-kms-and-appliance-configuration.html["Übersicht über die KMS-Appliance-Konfiguration"^] für weitere Informationen.
...
Die Laufwerksverschlüsselung kann für jede SED-SSD verwendet werden, die in einem SG100, SG1000, SG110, SG1100, SG120, SG1200, SGF6112, SGF6212, SG6100-CN oder SG6200-CN Rechenknoten oder Controller installiert ist.

* Für Service-Appliances sind die SSDs die Node-Root-Festplatten.
* Bei einem SG6100-CN- oder SG6200-CN-Controller werden die SSDs zum Caching verwendet.
* Bei einem SGF6112 oder SGF6212 sind die SSDs die Knoten-Root-Disks und werden für die primäre Speicherung von Objektdaten verwendet.
...
Wenn Sie die Laufwerkverschlüsselung für ein StorageGRID Gerät mit FIPS-Laufwerken aktivieren, wird die von den FIPS-Laufwerken bereitgestellte FIPS-Verschlüsselung zur Verschlüsselung ruhender Daten verwendet.

Sie können die Laufwerkverschlüsselung während der Erstinstallation des Geräts aktivieren, bevor Sie Grid Manager laden.  Sie können auch die Laufwerkverschlüsselung aktivieren oder Ihre Passphrase ändern, indem Sie das Gerät in den Wartungsmodus versetzen.
