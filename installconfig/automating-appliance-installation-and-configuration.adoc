...
---
= Automatisieren Sie die Installation und Konfiguration der StorageGRID-Appliance
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** LACP-Übertragungs-Hash-Richtlinie und PDU-Rate
...
* Eine JSON-Datei mit Konfigurationsdetails wird generiert. In Zusammenarbeit mit Ihrem NetApp Professional Services Consultant kann das link:https://configbuilder.netapp.com/Index.aspx["NetApp ConfigBuilder Tool"^] (Anmeldung erforderlich) zur Durchführung dieser Schritte verwendet werden:
+
...
. Suchen Sie im Abschnitt „JSON hochladen“ nach der JSON-Konfigurationsdatei, die Sie hochladen möchten, und wählen Sie sie aus.
+
